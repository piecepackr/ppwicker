...
#' @export
download_game_metadata <- function(update = FALSE) {
    ...
    games <- xml2::xml_find_all(doc, "//li/a")
    links <- xml2::xml_attr(games, "href")
    links <- links[-grep("GameTemplate", links)]
    raw_links <- gsub("ppwiki", "ppwiki/raw", links)
    targets <- paste0(cache_dir, "/txt/", basename(raw_links), ".txt")
